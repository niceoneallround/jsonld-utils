...
//
// Extract the objects of the specified type from the passed jsonld doc using the
...
function findObjects(inputDoc, type, optionsMap) {
  'use strict';

  var expandPromise, p1, p2;

  // if no context see if one in the optionsMap
  ...
  // create promise for the expand
  expandPromise = jsonld.promises.expand(inputDoc);
  p1 = expandPromise.then(
    function (expanded) { // ok
      console.log('FindObjects - expanded Promise.then: %j', expanded);
      return expanded;
    },

    function (reason) { // err
      assert.fail(util.format('FindObjects - Unexpected expand error:%s', reason));
    }
  );

  // create promise for the frame which is chained of the expand
  p2 = p1.then(
    function (expanded) {
      var framePromise,
          frame = {
      ...
      return framePromise.then(
        ...
        function (reason) { // err
          assert.fail(util.format('FindObjects - Unexpected frame error:%s', reason));
        }
      );
    },
