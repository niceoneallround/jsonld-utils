...
//  *@type - must be at least one type
...
function addType2Node(node, type) {
  ...
  assert(node, 'addType requires a node');
  assert(type, 'addType requires a type');
  ...
  if (util.isArray(type)) {
    ...
    if (!node['@type']) {
      node['@type'] = [];
    } else if (!util.isArray(node['@type'])) {
      node['@type'] = [node['@type']];
    }
