...
//
// The expanded format handles the following
// - #1 the property value is in the @value format. Example prop1: {'@value': '23'}
// - #2 the property value maybe wrapped by an array. Example prop1: [{'@value: '23}]
//
// The compact format
// - basic js. Example prop1: '23'
//
...
function getV(prop) {
  ...
  if (prop['@value']) {
    ...
    // expanded format 2
    assert((prop[0]['@value']), util.format('getV cannot handle array that does not have a @value %j', prop));
    return prop[0]['@value'];
  } else {
